...
def _normalize_events(
    ...
    if lat_col not in ev.columns:
        if "lat" in lower: ev = ev.rename(columns={lower["lat"]: lat_col})
        elif "latitude" in lower: ev = ev.rename(columns={lower["latitude"]: lat_col})
    if lon_col not in ev.columns:
        if "lon" in lower: ev = ev.rename(columns={lower["lon"]: lon_col})
        elif "longitude" in lower: ev = ev.rename(columns={lower["longitude"]: lon_col})
...
def prob_ge_k(lmbd: float, k: int) -> float:
    """Poisson P(X >= k), k küçükken kapalı form (k=1) hızlıdır."""
    l = float(max(lmbd, 0.0))
    if k <= 1:
        return 1.0 - np.exp(-l)
    # k>=2 için küçük bir seri toplamı:
    # P(X >= k) = 1 - sum_{i=0..k-1} e^-l l^i / i!
    terms = [np.exp(-l)]
    acc = terms[0]
    for i in range(1, k):
        terms.append(terms[-1] * l / i)
        acc += terms[-1]
    return float(max(0.0, 1.0 - acc))
...
def aggregate_fast(
    ...
    *,
    ...
    # Not: detaylı NR hesapları components/utils/hotspots.py içinde; burada çok hafif bir approx yapıyoruz.
    if events is not None and isinstance(events, pd.DataFrame) and not events.empty:
        ...
        decay = np.exp(-nr_lookback_h / max(nr_decay_h, 1e-6))
    ...
    def _tier(x: float) -> str:
        if x >= q66 and x > 0: return "Yüksek"
        if x >= q33 and x > 0: return "Orta"
        return "Hafif"
    ...
    # geo_df'ten neighborhood vb. varsa bırak
